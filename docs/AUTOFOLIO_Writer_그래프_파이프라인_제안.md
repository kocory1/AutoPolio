...
**호출 경로:** `POST /api/cover-letter/draft` (body: `questions[]` 다문항) → **Writer 그래프**를 문항별로 호출 (에셋 조회는 그래프 내부 `load_assets`) → `drafts[]` 반환. (전략 수립 없음)

**API 입력:** `job_id`(선택), `questions[]` (각 항목: question_text, max_chars 필수)  
**그래프 입력(문항 1개 기준):** `user_id`, `question`, `max_chars` (및 `job_parsed` 선택)  
**출력:** `drafts[]` (draft_id, question_text, answer, char_count)
...
[클라이언트] POST /api/cover-letter/draft
    body: { "job_id": "..." (선택),
            "questions": [ { "question_text": "...", "max_chars": 500 }, ... ] }
         │  다문항(3~5개) 한 번에 입력. user_id는 인증에서 추출.
         ▼
[서버] 1. job_id 있으면 jobs 테이블에서 job_parsed 조회
      2. 문항별 drafts 레코드 생성
      3. Writer 그래프를 문항별로 호출 (user_id, question, max_chars, job_parsed)
         → retrieve_samples → load_assets → generate_draft → self_consistency → format_output
      4. drafts[] (draft_id, question_text, answer, char_count) 반환
```
...
| **문항 입력** | 자소서 문항은 **유저 입력**. draft API body에 `questions[]` (각 항목: question_text, max_chars 필수) 필수. | max_chars만 필수. 채용공고 파싱 대상 아님. [AUTOFOLIO_채용공고파싱전략.md](AUTOFOLIO_채용공고파싱전략.md) |
...
- 1.11: PR 리뷰 반영. tone, min_chars 제거. draft_sessions/cover_letter_items → drafts 단일 테이블. draft API 연동에서 캐시 제거. 출력에서 draft_session_id 제거.
