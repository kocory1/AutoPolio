...
| 3 | **Inspector** | 3 | 초안, 에셋, (선택) 유저 수정본 | 보완 제안, Human-in-the-loop |
...
**용도:** 초안 + 에셋 → 보완 제안. 유저 수정 후 재첨삭(Human-in-the-loop).
...
| 노드 | 역할 | 입출력(State) |
|------|------|----------------|
| `load_draft` | 초안·에셋 로드 | 입력: `draft`, `assets` |
| `analyze` | LLM으로 보완점 분석 | 입력: `draft`, `assets`, `user_edited` / 출력: `suggestions` |
| `suggest` | 제안 반환 | 입력: `suggestions` / 출력: `suggestions` |
| `re_inspect` | 수정본 재검토 | 입력: `user_edited`, `round` / 출력: `round`, `draft` |
