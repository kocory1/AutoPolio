...
P1 (데이터) ── 제공 ──►  P2 (서비스 · LangGraph)
  · 합격 자소서 검색 API     · Writer 그래프: retrieve 노드에서 호출
...
**합격 자소서 검색이 4주인 이유:** 파이프라인상 검색 결과는 **자소서 작성 AI·첨삭 AI(LangGraph)** 로 들어가지만, **검색 API는 P1이 제공**하고 Writer/Inspector 그래프가 **호출**하는 구조라서, **API를 먼저 준비(4주)** 해 두어야 9주 Writer 그래프 구현 시 `retrieve_samples` 노드에서 사용할 수 있음. 4주로 두면 8주 스펙 정리까지 여유가 있음.
...
Job Fit는 단일 연산(공고 vs 프로필 유사도)이므로 그래프 없이 API 한 번 호출로 처리.
...
- 노드: `load_context` → `retrieve_samples`(P1 합격 자소서 검색 API 호출) → `generate_draft`(LLM) → `format_output`
- State: `strategy`, `assets`, `question`, `samples`, `draft`, `messages`(선택)
- 엣지: 선형. `retrieve_samples`에서 P1 API 호출.
...
- **API와 맞출 것:** `retrieve_samples`가 사용할 P1 검색 API 요청/응답 형식을 2주 차 API 스펙에 포함.
...
| **4** | ChromaDB 이전 완료, **합격 자소서 검색 API**(문항/회사/연도 필터) 구현 → 9주 Writer 그래프 `retrieve_samples`·Inspector에서 사용 | 레포 선택 UI/API, 기본 제외 경로(node_modules 등) 설정, 트리/블롭/커밋 수집 진입 준비 | 이력서 DB 검색 API 동작 / “선택 레포” 기준 수집 준비 |
| **5** | GitHub 수집 배치 설계: 선택 레포 트리/블롭/커밋 수집, 제외 경로 적용. RAPTOR 설계(단위·요약 단계·임베딩 저장) | Job Fit 점수화 설계: 공고 임베딩 vs User 프로필 임베딩, 유사도 지표. 채용 공고 파싱 스펙 정리·LLM 크롤링 1차 구현 | Phase 1·2 경계 정리, 수집·Job Fit 설계 완료 |
...
| 주차 | P1 | P2 | 마일스톤 |
|------|-----|-----|----------|
| **6** | GitHub 레포 트리/블롭/커밋 수집 배치 구현, 기본 제외 적용. RAPTOR 임베딩 파이프라인(선택 레포 기준) 설계·1차 구현 | Job Fit 유사도 계산 구현(공고·프로필 임베딩), 점수·순위 API. 자기소개서 문항 입력 UI/API(문항 리스트 저장, 문항별 메타) | Job Fit 1차 버전 동작 / 문항 입력 가능 |
...
| **8** | 합격 자소서 Retrieval API 정리(문항/회사/키워드), Writer 그래프 `retrieve_samples`에서 쓸 응답 형식 확정 | 전략 수립 그래프 E2E. **Writer 그래프** 설계 확정: State·노드(`load_context`, `retrieve_samples`, `generate_draft`, `format_output`), 프롬프트 초안 | 전략 그래프 E2E / Writer 그래프 스펙 확정 |
...
- **P2가 제공**
  ...
  - 채용 공고 파싱, Job Fit 점수·순위, 문항/전략, Writer/Inspector API.
- **문서**
