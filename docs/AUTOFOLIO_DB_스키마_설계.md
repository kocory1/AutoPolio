...
자소서 문항 단위 저장. 문항(question)과 초안(answer)을 하나의 인스턴스로 관리. API `draft_id` = `drafts.id`. `users.id` FK, `jobs.id` FK (nullable).
...
### 2.3 jobs (공고 파싱·직접입력 저장)

POST /api/jobs/parse 호출 시 항상 저장된다.  
**source_type=url:** url 컬럼 기준으로 신규 저장.  
...
RAPTOR 임베딩 시 파일·폴더가 어느 폴더에 속하는지 표현. **path 자체를 id로 사용**해 단일 진실원(SSoT). `selected_repos.id` FK. (팀 논의: [week3-oauth-repo-list.md](../week-issues/week3-oauth-repo-list.md) § asset_hierarchy path 설계 근거)


| 컬럼                 | 타입         | 설명                                                                        |
| ------------------ | ---------- | ------------------------------------------------------------------------- |
| `id`               | TEXT PK    | 경로 자체를 id로 사용. 예: `owner/repo/src/auth/login.py`. ChromaDB document id와 동일. GitHub Contents API용 path는 id에서 `owner/repo/` 제거해 유도. |
...
users
  ...
  │       └── asset_hierarchy (selected_repo_id FK, id=path SSoT)
...
- 1.7: asset_hierarchy 테이블 추가 (selected_repo_id FK, type). id=path SSoT. (week3 팀 논의 반영)
