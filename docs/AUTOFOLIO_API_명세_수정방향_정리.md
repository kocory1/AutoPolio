...
| 항목 | 수정 방향 | 반영 문서 |
|------|-----------|-----------|
| **전략 수립** | 별도 API 없음. 자소서 초안(draft) 요청 시 내부에서 전략 수립 그래프 호출 후 Writer로 이어짐. | LangGraph 설계, 파이프라인, RAG 파이프라인, 16주 계획표 |
| **채용 공고 파싱** | 추출 항목: **담당 업무 / 자격 요건 / 우대 사항 / 기업명 / 기업 인재상 / 포지션명** (6가지) | 채용공고파싱전략, 기획서, 파이프라인, 16주 계획표 |
...
| **GitHub 연동 해제** | **MVP에서는 불필요.** 로그아웃으로 충분. 연동 해제(disconnect)는 추후 확장 시 검토. | 기획서 제약·범위 |

---

## 2. API 명세 요약 (수정 반영 후)

- **인증:** GET /api/auth/github/login, callback, GET /api/auth/logout. (disconnect는 MVP 제외)
...
- **채용공고:** POST /api/jobs/parse → 담당업무/자격요건/우대사항/기업명/기업인재상/포지션명 추출.
- **Job Fit:** GET 또는 POST /api/job-fit (User DB + 공고 API 반환값 비교 → 점수).
- **자소서:** POST /api/cover-letter/draft (문항 → 내부 전략 수립 + Writer + 합격 자소서 모듈 → 초안), POST /api/cover-letter/inspect.
...
- 1.0 (2025-03-04): 수정 방향 반영, API 명세 요약 정리.
